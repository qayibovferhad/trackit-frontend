    ...
    <div className="rounded-xl border bg-white p-5 h-[500px] flex flex-col">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Team tasks sorted by priority</p>
        </div>
      </div>

      <div className="mt-4 flex gap-2">
        <Button onClick={() => setActiveFilter('upcoming')} variant={activeFilter === 'upcoming' ? "violet" : "secondary"} size="sm">
          {data?.meta.upcoming || 0} Upcoming
        </Button>
        <Button size="sm" variant={activeFilter === 'overdue' ? "violet" : "secondary"} onClick={() => setActiveFilter('overdue')}>
          {data?.meta.overdue || 0} Overdue
        </Button>
        <Button size="sm" variant={activeFilter === 'completed' ? "violet" : "secondary"} onClick={() => setActiveFilter('completed')}>
              ...
              <li key={task.id}>
                ...
                <div className="mt-2 ml-6">
                  <div className="flex items-center gap-4 flex-wrap">
                    {task.dueAt && (
                      <p className="text-sm text-gray-500">{formatDate(task.dueAt)}</p>
                    )}
                    {task?.tags && task.tags.length > 0 ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {task.tags.map(tag => (
                            <span key={tag} className="px-2 py-0.5 rounded text-xs bg-gray-100 uppercase">
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <Tag size={16} />
                        <span className="px-2 py-0.5 rounded text-xs bg-gray-100">No Tag</span>
                      </div>
                    )}
                  </div>
                </div>
